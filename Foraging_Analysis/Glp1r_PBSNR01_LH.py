...
name = ['PBSNR01-LH', 'PBSNR01_BH', 'PBSNR02-LH']
behavfiles=[]
photomfiles=[]

for i in name:

    ind_files = glob.glob("/Volumes/Aelita/Glp1r_agonist/processed/selected_days/" + i + "/*RecBehav.mat")
    photomfiles_ind = glob.glob("/Volumes/Aelita/Glp1r_agonist/processed/selected_days/" + i + "/*AlignedPhoto.mat")
...
#%%
Group=['photometry','photometry', 'photometry' ]
...
#%%
plt.figure(figsize=(4,4))
plot_df=df_tot[1].groupby('session').apply(filter_session).reset_index(drop=True)
g7=sns.relplot(data = plot_df.query("session==7 and poke_number==2") , x = "frames", y = "dopamine", hue = "poke_number",  kind = "line")
